...
> - **2026-07-02 — Approved fix batch shipped** (8 commits, `3703834`…`5caa4c9`):
>   **C1 RESOLVED** — rewind/scroll-back timers separated, dropped auto-advances now reschedule (`3703834`); **visible Continue buttons restored on every step**, full-width on mobile, brand-red on the dark path section (`487689f`); **C4 RESOLVED** — Areas row wraps, false "keep scrolling/keep moving" captions removed, duplicate patch block deleted (`d72e91b`); **details step no longer auto-advances** — explicit "Build My Smart Move Brief" button (`a1e2e78`); **C5 RESOLVED** — partial lead POST after contact step, marked `metadata.submissionType='partial_contact'`, fire-and-forget (`f275036`); **UTM/fbclid capture** into `metadata.tracking` on partial + final (`6e32976`); **C3 RESOLVED** — privacy policy page wired to both footers (`0f5b126`); **favicon/apple-touch-icon** with the JW mark (`5caa4c9`).
>   **Verified** (Playwright, local serve of production code): all six paths completed end-to-end at 390px using only the visible buttons; scroll-up-after-contact stranding scenario now recovers; details step stays put after required fields complete; **zero horizontal overflow at 360/390/430/820/1440 on every step**; 11 partial + 10 final submissions captured with correct payloads and tracking params; no console errors. After-screenshots + raw results: [`audit-evidence/after/`](audit-evidence/after/). Production HubSpot/Resend delivery still needs one live submission check after merge (sandbox cannot reach external APIs).
