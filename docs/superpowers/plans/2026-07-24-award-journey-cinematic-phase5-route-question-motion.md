...
**A correction made mid-execution, after Task 1 was first dispatched:** the original version of this plan (below) declared `index.html` off-limits and assumed the three containers Phase 1 scaffolded as siblings of `#route-detail-fields` could stay siblings, on the premise that `tests/verify.mjs` queries `.detail-field` globally. That premise was wrong — `tests/verify.mjs`'s `fillDetails()` scopes its lookup to `document.getElementById('route-detail-fields').querySelectorAll('.detail-field')`, a query that does not reach sibling elements. Simply nesting the containers inside `#route-detail-fields` was also not a safe fix on its own: `assets/js/app.js`'s `resetSmartMoveState()` (permanently off-limits) unconditionally clears `#route-detail-fields`'s `innerHTML` on Start Over, which would delete nested containers outright and silently break the form on any second pass. The actual fix, dispatched as a correction to Task 1: move the three containers (plus a new fourth, `#detail-scene-scroll`, for the fallback bucket) to be children of `#route-detail-fields` in `index.html`, and have `renderRouteDetails()` find-or-create each one on every call (reusing an existing, correctly-parented container across same-visit re-renders so the entrance animation doesn't replay on every checkbox click, but transparently recreating it if a reset wiped it clean — which correctly replays the animation on a genuine new pass through the form). The task steps below are left as originally written for the historical record; the actual code that shipped reflects this correction, not the original snippets.
